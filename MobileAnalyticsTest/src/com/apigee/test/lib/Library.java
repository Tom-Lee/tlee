...
/*
 * Purpose: a list of methods to validate the mobile analytics pages, login, and logout
 * Author: Tom Lee
 * @Apigee Corporation
 */
public class Library{
	...
	public void appMobileAnalytics(Selenium browser, Log log){
		...
		try{
			...
			if (util.waitforElementText(browser,util.getXpath("mobileAnalytics_id","DashboradPage"), log)){
				...
				util.waitTime(5);	
				if (util.validateTest(1,util.getXpath("addapp_id","DashboradPage"), browser, log)){
					browser.click(util.getXpath("addapp_id","DashboradPage"));
					if(util.waitforElementText(browser, util.getXpath("addapp_id","DashboradPage"), log)){
						browser.type(util.getXpath("addinput_id", "DashboradPage"),util.getXpath("input_id","DashboradPage"));
						browser.click(util.getXpath("add_id", "DashboradPage"));
						if (util.validateTest(2,util.getXpath("text_id", "DashboradPage"), browser, log)){
							browser.click(util.getXpath("mb_id", "DashboradPage"));						
						}
					}else{
						log.error("Error: cannot add new app");
					}
				}else if (util.validateTest(2,"App Overview",browser, log )){					
					if (util.waitforElementText(browser,util.getXpath("monitor_id","ActionPage"), log)){
						...
						log.error("Error: cannot access monitor app page");
					}
				}
				util.waitTime(5);
	...
	public void goBacktoDashboard(Selenium browser, Log log){
		...
		try {
			...
			log.error("==>Error during goBacktoDashboard" + e.getMessage());
		}
	}
}
