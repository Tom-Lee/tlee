...
public class Library{
					...
					if (util.waitforElementText(browser,util.getXpath("sendHistory_id", "AppService"), log)){
						...
						if (util.waitforElementText(browser, util.getXpath("notifiercheck_id", "AppService"), log)){
							...
							if (!util.validateTest(2, util.getXpath("notifierinput_id", "AppService"), browser, log))
								...
								log.error("Error: failed to delete the notifier");
						}
					}						
				}else{
	 ...
	 * Find uuid 
	 */
	private ArrayList<String> findUUID(Selenium browser,Log log){
		ArrayList<String>uuid=new ArrayList<String>();
		int i=2;
		try{
			while (util.validateTest(1,util.getXpath("finduuid_id", "AppService")+"tr["+i+"]/td[3]" , browser, log)){
				uuid.add(browser.getText(util.getXpath("finduuid_id", "AppService")+"tr["+i+"]/td[3]"));
				i+=2;
			}
			if (uuid.size()==0){
				Exception e = new Exception("Cannot find uuid");
                throw e;
			}
		}catch(Exception e){
			log.error("Error: exception in findUUID "+e.getMessage());
		}
		return uuid;
	}
	
	/**
	 * Access window to perform action 
	 */
	private void accessWindows(Selenium browser, Log log, Boolean flag) throws InterruptedException{
		...
		try{
			...
			if (feedWinId!=null){
				...
				log.error("Error: failed to reterieve find window");
			}
			util.waitTime(2);;
			...
			if (!winFocus[0].isEmpty()) {
				...
				log.error("Error: failed to reterieve the main window");
			}
		} catch (Exception e) {		
